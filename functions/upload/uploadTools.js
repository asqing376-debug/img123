...
/**
 * 根据页面配置的 urlPrefix 或当前请求 origin 生成上传后的返回链接
 * @param {string} urlOrigin - 当前请求的 origin，如 https://example.com
 * @param {string} [urlPrefix] - 后台「默认URL前缀」配置，如 https://img.a.com/file/
 * @param {string} fullId - 文件 ID
 * @param {string} returnFormat - 'default' 或 'full'
 * @returns {string} 返回链接
 */
export function getReturnLink(urlOrigin, urlPrefix, fullId, returnFormat) {
    const prefix = typeof urlPrefix === 'string' ? urlPrefix.trim() : '';
    if (prefix) {
        const base = prefix.replace(/\/$/, '');
        const fullUrl = base.endsWith('/file') ? base + '/' + fullId : base + '/file/' + fullId;
        return fullUrl;
    }
    if (returnFormat === 'full') {
        return `${urlOrigin}/file/${fullId}`;
    }
    return `/file/${fullId}`;
}
