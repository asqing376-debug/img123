/* ========== 分块合并处理 ========== */
import { createResponse, getUploadIp, getIPAddress, selectConsistentChannel, buildUniqueFileId, endUpload, getReturnLink } from './uploadTools';
...
async function mergeR2ChunksInfo(context, uploadId, completedChunks, metadata) {
    ...
    try {
        ...
        // 更新返回链接（优先使用后台「默认URL前缀」）
        const returnFormat = url.searchParams.get('returnFormat') || 'default';
        const urlPrefix = context.pageConfig?.config?.find(c => c.id === 'urlPrefix')?.value;
        const updatedReturnLink = getReturnLink(url.origin, urlPrefix, finalFileId, returnFormat);
