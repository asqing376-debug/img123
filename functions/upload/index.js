...
import {
    ...
    moderateContent, purgeCDNCache, isBlockedUploadIp, buildUniqueFileId, endUpload, getImageDimensions, getReturnLink
} from "./uploadTools";
...
async function processFileUpload(context, formdata = null) {
    ...
    // 获得返回链接格式, default为返回/file/id, full为返回完整链接；优先使用后台「默认URL前缀」
    const returnFormat = url.searchParams.get('returnFormat') || 'default';
    const urlPrefix = context.pageConfig?.config?.find(c => c.id === 'urlPrefix')?.value;
    const returnLink = getReturnLink(url.origin, urlPrefix, fullId, returnFormat);
